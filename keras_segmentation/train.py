...
def train(
		...
		load_weights=None,
		steps_per_epoch=10,
		optimizer_name='SGD'):  # optimizer_name='adadelta'):

	if isinstance(model, six.string_types):  # check if user gives model name instead of the model object
	...
	print(train_images, train_annotations, batch_size, n_classes,
	...
	if validate:
		val_gen = image_segmentation_generator(val_images, val_annotations, val_annotations,
											val_batch_size, n_classes , input_height, input_width, output_height, output_width)
	...
	if not validate:
		...
		for ep in range(epochs):
			print("Starting Epoch ", ep)
			model.fit_generator(train_gen, steps_per_epoch, validation_data=val_gen, validation_steps=10, epochs=1)
			if not checkpoints_path is None:
				model.save_weights(checkpoints_path + "." + str(ep))
				print("saved ", checkpoints_path + ".model." + str(ep))
			print("Finished Epoch", ep)
	...
	#
	# if not validate:
	# 	model.fit_generator(train_gen, steps_per_epoch, epochs=10)
	# 	if not checkpoints_path is None:
	# 		model.save_weights(checkpoints_path)
	# 		print("saved ", checkpoints_path + ".model")
	#
	# else:
	# 	model.fit_generator(train_gen, steps_per_epoch, validation_data=val_gen, validation_steps=10,  epochs=10)
	# 	if not checkpoints_path is None:
	# 		model.save_weights(checkpoints_path)
	# 		print("saved ", checkpoints_path + ".model")
